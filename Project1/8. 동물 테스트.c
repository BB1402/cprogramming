...
//시작 화면
	...
	printf("어느 날 잠에서 깬 당신은\n 내 몸이 인간의 몸이 아님을 느낍니다.\n\n 숲 속의 동물이 된 나는 무엇을 하고 있을까요?\n");
	int select;
	printf("1. 숲속으로 떠나기");
	scanf_s("%d", &select);
...
//[2번 문항]
	...
	printf("\t2. 미리 준비해 둔 옷과 가방을 챙겨 나간다.\n");
...
//[3번 문항]
	...
	printf("\t2. 지도를 한 번 슥 보고 발길 닿는 대로 간다.\n");
...
//[6번 문항]
	printf("\t사람들이 나무를 베어 가\n 소중한 숲 속이 황량해진 것을 본다면\n");
...
//[7번 문항]
	...
	printf("\t1. \"우리가 개선할 수 있는 게 뭘까?\"\n현실적으로 고민해본다.\n");
	printf("\t2. 앞으로 숲도... 지구도 다 망가지는 거 아냐?\"\n미래를 걱정한다.\n");
...
//[9번 문항]
	...
	printf("\t1. 그 동안 내가 보고 느낀 것들을 적극적으로 설명한다.\n");
...
//[12번 문항]
	printf("\t숲 속 체험이 끝나고 현실에 돌아와 일기를 쓴다면,\n");
	printf("\t1. 숲 속에서 겪었던 일들을 자세히 묘사해서 쓴다.\n");
...
//반복문
//12번 반복을 진행하는 코드(i가 0에서 시작해서 작업이 끝날 때마다 1씩 i거 증가, i가 12보다 작을 동안 반복)
	int total = 0;
	for (int i = 0; i < 12; i++)
	{
		total += selected[i];
...
//12번 반복을 진행하는 코드(i가 0에서 시작해서 작업이 끝날 때마다 1씩 i거 증가, i가 12보다 작을 동안 반복)
	...
	return 0;
>>>>>>> 9699857c950f662274a1f32c9d0c88c60fa90a14
}